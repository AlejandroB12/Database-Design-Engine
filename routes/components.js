      ...
      for (const ref of (t.refs || [])) {
        ...
        if (points) {
          points = points.map((p, i) => {
            if (i === 0) return penetrateEndpoint(p, srcRect, srcColIdx);
            if (i === points.length - 1) return penetrateEndpoint(p, tgtRect, tgtColIdx);
            return p;
          });
    ...
    <div ref={containerRef}
      ...
      <div ref={canvasRef} className="diagram-canvas absolute" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`, transformOrigin: '0 0', width: canvasBounds.width, height: canvasBounds.height }}>
         <svg className="absolute inset-0 w-full h-full diagram-svg" style={{ pointerEvents: 'none', overflow: 'visible', zIndex: 5 }}>
          ...
          {tables.length > 0 && <rect x="0" y="0" width={canvasBounds.width} height={canvasBounds.height} fill="rgba(139, 148, 158, 0.03)" stroke="rgba(139, 148, 158, 0.5)" strokeWidth="2" strokeDasharray="10 6" rx="6" ry="6" style={{ pointerEvents: 'none' }} />}
                  ...
                  return (
                    <g key={`h-${pi}`}>
                      <circle cx={p.x} cy={p.y} r={isEndpoint ? 6 : 8}
                        ...
                        stroke={isEndpoint ? '#8b949e' : '#a371f7'}
      ...
      {tables.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="text-center opacity-60">
            ...
            <h2 className="text-xl font-bold text-[#c9d1d9] mb-2">Database Modeler</h2>
